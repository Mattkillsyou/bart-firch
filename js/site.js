...
/* ---------- WebGL ---------- */
const GL = window.__GL = { ready: false, planes: [], fade: 1, fadeTarget: 1, mouse: new THREE.Vector2(0, 0), mouseT: new THREE.Vector2(0, 0), vel: 0 };
...
uniform float uHover, uVel, uReveal, uTime, uWarm;
varying vec2 vUv;
uniform vec2 uFocus;
...
const grainShader = {
  uniforms: { tDiffuse: { value: null }, uTime: { value: 0 }, uAmount: { value: 0.035 }, uRes: { value: new THREE.Vector2(1, 1) }, uGlow: { value: 1 } },
  ...
  void main(){
    ...
    c.rgb = c.rgb * vig + vec3(0.07, 0.045, 0.02) * glow * uGlow + g;
    gl_FragColor = c;
  }`,
};
...
function initGL(manager) {
  ...
  document.querySelectorAll('.gl-fig[data-gl]').forEach((fig) => {
    ...
    const tex = texLoader.load(img.currentSrc || img.src);
    tex.colorSpace = THREE.SRGBColorSpace;
    tex.minFilter = THREE.LinearFilter; tex.generateMipmaps = false; tex.anisotropy = Math.min(4, renderer.capabilities.getMaxAnisotropy());
    const mat = new THREE.ShaderMaterial({
      vertexShader: planeVert, fragmentShader: planeFrag, transparent: true,
      uniforms: {
        uTex: { value: tex }, uPlane: { value: new THREE.Vector2(1, 1) }, uImage: { value: new THREE.Vector2(+img.getAttribute('width') || 3, +img.getAttribute('height') || 2) },
        uMouse: { value: new THREE.Vector2(0.5, 0.5) }, uHover: { value: 0 }, uVel: { value: 0 }, uReveal: { value: 0 }, uTime: { value: 0 }, uWarm: { value: 0.6 },
        uFocus: { value: (() => { const f = (fig.dataset.focus || '0.5 0.5').split(/\s+/).map(Number); return new THREE.Vector2(f[0], 1 - f[1]); })() },
      },
    });
    const mesh = new THREE.Mesh(planeGeo, mat);
    mesh.frustumCulled = false;
    scene.add(mesh);
    const plane = { fig, mesh, mat, hover: 0, mouse: new THREE.Vector2(0.5, 0.5) };
...
function onResize() {
  ...
  grainPass.uniforms.uRes.value.set(w * renderer.getPixelRatio(), h * renderer.getPixelRatio());
...
if (useGL) initGL(manager);
else { assetsDone = true; }
setProgress(0.15);
let fontsDone = false;
(document.fonts ? document.fonts.ready : Promise.resolve()).then(() => { fontsDone = true; maybeStart(); });
setTimeout(() => { assetsDone = true; fontsDone = true; maybeStart(); }, 7000);
...
function start() {
  setProgress(1);
  if (useGL) gsap.ticker.add(render);
...
function setupScroll(arriveFig) {
  ...
  /* photo reveals */
  GL.planes.forEach((p) => {
    const reveal = () => gsap.to(p.mat.uniforms.uReveal, { value: 1, duration: 1.7, ease: 'expo.out' });
    if (p.fig === arriveFig) { gsap.delayedCall(0.9, reveal); return; }
    ScrollTrigger.create({ trigger: p.fig, start: 'top 88%', once: true, onEnter: reveal });
  });
  if (!useGL) document.querySelectorAll('.gl-fig img').forEach((i) => { i.style.opacity = 1; });
  ...
  if (reduced) { ScrollTrigger.getAll().forEach((s) => s.kill()); gsap.set('.wi, .reveal-line, .arrive-copy', { clearProps: 'all' }); gsap.set(arriveFig, { '--p': 1 }); }
}
...
function setupForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!name || !email || !msg || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) { note.textContent = 'Add your name, a working email, and a line about what is going on.'; return; }
    note.textContent = 'Thanks, ' + name.split(' ')[0] + '. I will be in touch.';
    form.reset();
  });
}
